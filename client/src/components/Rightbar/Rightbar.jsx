...
const Rightsidebar = ({ cstart }) => {
    ...
    const savegame = async (e) => {
        ...
        try {
            const data = await axios.post("https://snake-ladder-server.onrender.com/api/v1/savegame/", {
    ...
    const load = async () => {
        try {
            const data = await axios.post("https://snake-ladder-server.onrender.com/api/v1/savegame/loadgame/", {
                email: localStorage.getItem("email")
            }).then((res) => {
    ...
    return (
        <>
            <AiOutlineMenuFold className={`${show ? "hidden" : ""} cursor-pointer float-right right-0 top-0 mr-2 mt-2 h-[30px] w-[30px] `} onClick={() => cshow(!show)} />
            ...
            <div className={`${show ? "" : "hidden"} border border-solid px-[10px] border-black border-x-[3px] right-0 top-0  float-right w-[260px] bg-slate-400 h-[100%] mr-[0px] mt-0px absolute block z-[1000000000]`}>
                                            ...
                                            <li className='bg-green-300 text-black' onClick={() => {
                                                ...
                                                <span className='pl-[10px] flex text-center justify-between'>
                                                    ...
                                                    <ImBin className='hover:text-red-500 float-right right-0 text-[20px] color-black hover:opacity-100' onClick={async () => {
                                                        try {
                                                            const data = await axios.post("https://snake-ladder-server.onrender.com/api/v1/savegame/deletegame", {
                                                                email: localStorage.getItem("email"),
                                                                index: j
                                                            }).then((res) => {
                                                                var str = res.data.msg;
